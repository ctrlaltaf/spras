...
class LocalNeighborhood(PRM):
    ...
    def generate_inputs(data, filename_map):
        ...
        if data.contains_node_columns('prize'):
            ...
            node_df = data.request_node_columns(['sources'])
        elif data.contains_node_columns('targets'):
            node_df = data.request_node_column (['targets'])
    ...
    def run(nodes=None, network=None, output_file=None, singularity=False):
        ...
        command = [
            ...
            'ctrlaltaf/local-neighborhood python local_neighborhood.py' ,
        ...
        out = run_container(
            container_framework, "ctrlaltaf/local-neighborhood", command, volumes, work_dir
        )
        print(out)
